...
def HKDF(key, length, appInfo=""):
    # implements RFC 5869, some
    # parts from https://github.com/MirkoDziadzka/pyhkdf
    ...
    while len(keyStream) < length:
        keyBlock = hmac.new(
            key, 
            msg=keyBlock+appInfo+chr(blockIndex),
...
class WhatsAppWebClient:
    ...
    # maps message tags (provided by WhatsApp)
    # to more information (description and callback)
    messageQueue = {}
    loginInfo = {
        "clientId": None,
        "serverRef": None,
        ...
        try:
            ...
            if messageTag in self.messageQueue:
                ...
                if pend["desc"] == "_status":
                    if messageContent[0] == 'Pong' and messageContent[1]:
                        pend["callback"](
                            {"Connected": True,
                    ...
                    qrCodeContents = self.loginInfo["serverRef"] + ","\
                        + base64.b64encode(
                            self.loginInfo["publicKey"].serialize()) + ","\
                    ...
                    if messageContent != "":
                        ...
                        try:
                            ...
                            self.onMessageCallback["func"](
                                processedData, self.onMessageCallback,
                    ...
                    if isinstance(jsonObj, list) and len(jsonObj) > 0:					# check if the result is an array
                        ...
                        if jsonObj[0] == "Conn":
                            ...
                            eprint(
                                "private key            : ",
                                base64.b64encode(
                                    self.loginInfo["privateKey"].serialize()))
                            eprint(
                                "secret                 : ",
                                base64.b64encode(self.connInfo["secret"]))
                            eprint(
                                "shared secret          : ",
                                base64.b64encode(self.connInfo["sharedSecret"]))
                            eprint(
                                "shared secret expanded : ",
                                base64.b64encode(
                                    self.connInfo["sharedSecretExpanded"]))
                            eprint(
                                "hmac validation        : ",
                                base64.b64encode(hmacValidation))
                            eprint(
                                "keys encrypted         : ",
                                base64.b64encode(keysEncrypted))
                            eprint(
                                "keys decrypted         : ",
                                base64.b64encode(keysDecrypted))
                            msg = '''
                                set connection info:
                                client, server and browser token;
                                secret, shared secret, enc key, mac key
                            '''
                            eprint(msg)
                            eprint(
                                "logged in as "
    ...
    def connect(self):
        ...
        # self.activeWs.run_forever(
        #   http_proxy_host="127.0.0.1", http_proxy_port=1087);
    ...
    def generateQRCode(self, callback=None):
        ...
        message = messageTag + ',["admin","init",['\
            + WHATSAPP_WEB_VERSION + '],["Chromium at '\
            + datetime.datetime.now().isoformat() + '","Chromium"],"'\
    ...
    def sendTextMessage(self, number, text):
        ...
        messageParams = {
            "key": {
                "fromMe": True, "remoteJid": number + "@s.whatsapp.net",
                "id": messageId},
            "messageTimestamp": getTimestamp(),
            "status": 1, "message": {"conversation": text}}
        msgData = [
            "action",
            {"type": "relay",
    ...
    def disconnect(self):
        # WhatsApp server closes connection
        # automatically when client wants to disconnect
